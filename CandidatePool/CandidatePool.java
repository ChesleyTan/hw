...
public class CandidatePool {
    ...
    public ArrayList<Candidate> getCandidatesForPosition( String pos ) {
    	ArrayList<Candidate> retPool = new ArrayList<Candidate>();
		for (int i = 0;i < pool.size();i++){
			if (pool.get(i).getPosition().equals(pos)){
				retPool.add(pool.get(i));
			}
		}
		return retPool;
	}
    ...
    public int removeCandidatesForPosition( String pos ) {
		...
		for (int i = 0;i < pool.size();i++){
			if (pool.get(i).getPosition().equals(pos)){
				pool.remove(i);
				numRemoved++;
				i--;//Account for size() change when a candidate is removed. Index values are moved down, so index is decremented to check the new element in this position.
			}
		}
		return numRemoved;
	}
